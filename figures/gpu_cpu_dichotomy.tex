...
\begin{tikzpicture}[
  ...
  \node (gpumem_name) [draw=none,font=\huge] at($(gpu.south)!.5!(gpu_ram.north) + (2,0)$) {$\sim\unit[700]{GB/s}$};

  \node (cpumem_arrow) [double arrow, draw, line width=1pt,inner xsep=.6cm,font=\Large,rotate=-90] at($(cpu.south)!.5!(cpu_ram.north)$) {};
  \node (cpumem_name) [draw=none,font=\huge] at($(cpu.south)!.5!(cpu_ram.north) + (-2,0)$) {$\sim\unit[70]{GB/s}$};
